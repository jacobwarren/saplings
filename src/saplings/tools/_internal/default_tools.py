...
class DuckDuckGoSearchTool(Tool):
    ...
    def __init__(
        ...
        max_results: int = 10,
        name: str = "web_search",
        description: str = "Performs a DuckDuckGo web search and returns the top search results.",
    ) -> None:
        """
...
class GoogleSearchTool(Tool):
    ...
    def __init__(
        ...
        def google_search(query: str, filter_year: int | None = None) -> str:
            """
            ...
            if self.organic_key not in results:
                if filter_year is not None:
                    return f"No results found for query: '{query}' with filtering on year={filter_year}. Use a less restrictive query or do not filter on year."
                return f"No results found for query: '{query}'. Use a less restrictive query."
    ...
    def __init__(
        ...
        def visit_webpage(url: str) -> str:
            ...
            try:
                ...
                markdown_content = md_convert(response.text).strip()
        ...
        def wikipedia_search(query: str) -> str:
            ...
            try:
                ...
                try:
                    ...
                    msg = (
                        "You must install `wikipedia-api` to use this tool. "
                        "Run `pip install saplings[tools]`."
                    )
                    raise ImportError(msg)
            except Exception as e:
                msg = f"Error importing required packages: {e}"
                raise ImportError(msg)

            if not self.user_agent:
                msg = "User-agent is required. Provide a meaningful identifier for your project."
                raise ValueError(msg)
            ...
            if self.extract_format not in extract_format_map:
                msg = "Invalid extract_format. Choose between 'WIKI' or 'HTML'."
                raise ValueError(msg)

            wiki_format = extract_format_map[self.extract_format]

            wiki = wikipediaapi.Wikipedia(
                user_agent=self.user_agent, language=self.language, extract_format=wiki_format
            )
...
class SpeechToTextTool(Tool):
    ...
    def __init__(
        ...
        description: str = "Transcribes audio into text.",
    ) -> None:
        """
        ...
        def transcribe_audio(audio_path: str) -> str:
            ...
            try:
                ...
                try:
                    ...
                    msg = (
                        "You must install `transformers` and `torch` to use this tool. "
                        "Run `pip install transformers torch`."
                    )
                    raise ImportError(msg)

                # Import librosa and torch lazily
                try:
                    import librosa  # type: ignore[import-not-found]
                except ImportError:
                    msg = "You must install `librosa` to use this tool. Run `pip install saplings[tools]`."
                    raise ImportError(msg)

                try:
                    import torch
                except ImportError:
                    msg = "You must install `torch` to use this tool. Run `pip install saplings[gasa]`."
                    raise ImportError(msg)
            except Exception as e:
                msg = f"Error importing required packages: {e}"
                raise ImportError(msg)
            ...
            try:
                ...
                # Process the audio - handle the case where processor might be a tuple
                if isinstance(processor, tuple):
                    # If processor is a tuple, use the first element (assuming it's the actual processor)
                    actual_processor = processor[0]
                    input_features = actual_processor(
                ...
                # Decode the transcription - handle the case where processor might be a tuple
                if isinstance(processor, tuple):
                    # If processor is a tuple, use the first element (assuming it's the actual processor)
                    actual_processor = processor[0]
                    return actual_processor.batch_decode(generated_ids, skip_special_tokens=True)[0]
                # Normal case
