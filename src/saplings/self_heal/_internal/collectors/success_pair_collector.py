...
from saplings.self_heal._internal.interfaces import ISuccessPairCollector
...
if TYPE_CHECKING:
    # Define a Protocol for Patch to avoid circular imports
    from typing import Protocol

    class Patch(Protocol):
        """Protocol for Patch class."""
...
class SuccessPairCollector(ISuccessPairCollector):
    ...
    def _sanitize_data(self, input_text: str, output_text: str) -> tuple[str, str]:
        """
        Sanitize sensitive data from input and output text.
        ...
        Returns:
        -------
            Tuple of (sanitized_input, sanitized_output)
        ...
        for pattern, replacement in sensitive_patterns:
            sanitized_input = re.sub(pattern, replacement, sanitized_input, flags=re.IGNORECASE)
    ...
    async def collect(
        ...
        sanitized_input, sanitized_output = self._sanitize_data(input_text, output_text)
        pair["input_text"] = sanitized_input
        pair["output_text"] = sanitized_output
