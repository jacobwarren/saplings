...
class MemoryManager(IMemoryManager):
    ...
    def __init__(
        ...
        if memory_store is not None:
            ...
            # These will be initialized only if memory_path is provided
            self._vector_store = None
            self._indexer = None
            ...
            memory_config.vector_store.persist_directory = memory_path

            self._memory_store = MemoryStore(config=memory_config)
            self._dependency_graph = DependencyGraph()
            self._vector_store = get_vector_store(config=memory_config)
            self._indexer = get_indexer(config=memory_config)
        else:
            msg = "Either memory_path or memory_store must be provided"
            raise ValueError(msg)

        logger.info("MemoryManager initialised at %s", memory_path)

    @property
    def memory_store(self):
        """Get the memory store."""
        if self._memory_store is None:
            msg = "Memory store is not initialized"
            raise ValueError(msg)
    ...
    def dependency_graph(self):
        ...
        if self._dependency_graph is None:
            msg = "Dependency graph is not initialized"
            raise ValueError(msg)
    ...
    def add_document(
        ...
        *,
        ...
        if hasattr(self.memory_store, "add_document") and hasattr(
            self.memory_store, "get_document"
        ):
            return self.memory_store.add_document(
                content=content, metadata=metadata, embedding=embedding, document_id=document_id
            )

        # Otherwise, use the async version
        import asyncio

        return asyncio.run(
            self._add_document_async(
                content=content,
                metadata=metadata,
                embedding=embedding,
        ...
        *,
        ...
        try:
            ...
            async def _add_and_index():
                ...
                if use_indexer and self.indexer is not None:
    ...
    def add_documents_from_directory(
        ...
        *,
        ...
        if hasattr(self.memory_store, "add_documents_from_directory") and callable(
            ...
            return self.memory_store.add_documents_from_directory(  # type: ignore
        ...
        return asyncio.run(
            self._add_documents_from_directory_async(
                directory=directory, extension=extension, use_indexer=use_indexer, timeout=timeout
            )
        )

    async def _add_documents_from_directory_async(
        self,
        directory: str,
        *,
        extension: str = ".txt",
        ...
        try:
            ...
            for filename in filenames:
                ...
                # Add document using the async version directly
                document = await self._add_document_async(
    ...
    def get_document(
        ...
        if hasattr(self.memory_store, "get_document"):
            return self.memory_store.get_document(document_id)

        # Otherwise, use the async version
        import asyncio

        return asyncio.run(self._get_document_async(document_id, timeout=timeout))
    ...
    async def _get_document_async(
        ...
        try:
            ...
            return await run_in_executor(
                self.memory_store.get_document, document_id, timeout=timeout
            )
        finally:
            if span and self._trace_manager:
                self._trace_manager.end_span(span.span_id)
        ...
        try:
            ...
            def _get_documents():
                ...
                if self.vector_store is not None:
                    documents = self.vector_store.list()
                elif (
                    hasattr(self.memory_store, "vector_store")
                    and self.memory_store.vector_store is not None
                ):
            ...
            return await run_in_executor(_get_documents, timeout=timeout)
        finally:
            if span and self._trace_manager:
                self._trace_manager.end_span(span.span_id)
        ...
        if hasattr(self.memory_store, "search") and callable(
            ...
            try:
                ...
                if len(param_names) > 0 and param_names[0] == "query_embedding":
                    ...
                    logger.warning("Memory store search requires an embedding, not a string query")
    ...
    def search_by_embedding(
        ...
        return asyncio.run(
            self._search_by_embedding_async(
                embedding=embedding,
                limit=limit,
                threshold=threshold,
                filter_func=filter_func,
    ...
    async def _search_by_embedding_async(
        ...
        try:
            ...
            # Run in executor with timeout
            return await run_in_executor(_search_by_embedding, timeout=timeout)
        finally:
            if span and self._trace_manager:
                self._trace_manager.end_span(span.span_id)
    ...
    def delete_document(
        ...
        return asyncio.run(self._delete_document_async(document_id=document_id, timeout=timeout))
    ...
    async def _update_document_async(
        ...
        Returns:
        -------
            Optional[Document]: The updated document if successful
        ...
        try:
            ...
            def _update_document():
                ...
                if metadata:
                    ...
                    elif hasattr(metadata, "__dict__"):  # pragma: no cover
                        # This branch is technically unreachable since we check for dict above
                        # and metadata can only be dict or None at this point
                        doc_metadata = metadata.__dict__.copy()
                ...
                if hasattr(self.memory_store, "update_document"):
                    try:
                        ...
                        if hasattr(self.memory_store, "get_document"):
                            ...
                            self.memory_store.update_document(document_id, content, doc_metadata)
                            ...
                            self.memory_store.add_document(
                                content=content, metadata=doc_metadata, document_id=document_id
                            )
                        else:
                ...
                if self.vector_store is not None:
                    ...
                    try:
                        ...
                        if hasattr(self.vector_store, "add_documents"):
                            ...
                            self.vector_store.add_texts([content], [doc.metadata], [document_id])  # type: ignore
                        else:
            ...
            # Run in executor with timeout
            return await run_in_executor(_update_document, timeout=timeout)
        finally:
            if span and self._trace_manager:
                self._trace_manager.end_span(span.span_id)
    ...
    def count(
        ...
        documents = asyncio.run(self.get_documents(filter_func=filter_func, timeout=timeout))
    ...
    def clear(
        ...
        return asyncio.run(self._clear_async(timeout=timeout))
    ...
    def save(
        ...
        return asyncio.run(self._save_async(directory=directory, timeout=timeout))
        ...
        if hasattr(self.memory_store, "load") and callable(
            ...
            if directory is None:
                if self._memory_path is None:
                    ...
                    return False
                directory = self._memory_path

            self.memory_store.load(directory)
            return True

        # Otherwise, use the async version
        import asyncio

        return asyncio.run(self._load_async(directory=directory, timeout=timeout))
