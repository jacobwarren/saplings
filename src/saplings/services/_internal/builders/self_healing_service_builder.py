...
This builder supports lazy initialization and event-based error handling.
"""
...
from saplings.core.initialization import initialize_service
...
class SelfHealingServiceBuilder(ServiceBuilder[SelfHealingService]):
    ...
    def __init__(self) -> None:
        ...
        # Dependencies are optional for lazy initialization
    ...
    def build(self) -> SelfHealingService:
        ...
        This method creates a new SelfHealingService instance with the configured
        dependencies but does not initialize it. The service will be initialized
        lazily when it is first used.

        Returns
        -------
            The self-healing service instance (not yet initialized)

        """
        # Create the service instance
        service = super().build()

        # Register the service with the initialization system
        # This allows the service to be initialized lazily and participate
        # in the event-based initialization system
        try:
            initialize_service(service, "self_healing_service")
            logger.debug("Self-healing service registered with initialization system")
        except Exception as e:
            logger.warning(
                f"Failed to register self-healing service with initialization system: {e}"
            )
