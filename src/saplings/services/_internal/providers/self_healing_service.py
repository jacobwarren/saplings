...
from saplings.api.core import ISelfHealingService, SelfHealingConfig, SelfHealingResult
...
class SelfHealingService(ISelfHealingService):
    ...
    This service supports lazy initialization and event-based error handling.
    It will initialize its dependencies on-demand and register with the
    monitoring service for error events.
    """
    ...
    def initialize(self) -> None:
        ...
        This method initializes the service and its dependencies. It is called
        automatically when the service is first used, but can also be called
        explicitly if needed.
        """
        if self._initialized:
            return

        logger.debug("Initializing self-healing service")

        # Register with the monitoring service if available
        if self._trace_manager and hasattr(self._trace_manager, "register_service"):
            self._trace_manager.register_service("self_healing_service", self)
            logger.debug("Self-healing service registered with monitoring service")

        # Mark as initialized
        self._initialized = True
        logger.debug("Self-healing service initialized")
    ...
    async def heal(
        ...
        effective_config = config or SelfHealingConfig()

        # Check if self-healing is enabled
        if not self.enabled or not effective_config.enabled:
            return SelfHealingResult(success=False, metadata={"reason": "Self-healing is disabled"})

        span = None
        if self._trace_manager:
            span = self._trace_manager.start_span(
                name="SelfHealingService.heal",
                trace_id=trace_id,
                attributes={
                    "component": "self_healing_service",
                    "auto_apply": effective_config.auto_apply_patches,
                },
            )

        try:
            # Generate a patch
            patch_result = await self.generate_patch(
                failure_input=failure_input, failure_output=failure_output, trace_id=trace_id
            )
            ...
            if not patch_result.get("success", False):
                return SelfHealingResult(
                    ...
                    metadata={
                        ...
                        "details": patch_result.get("error", "Unknown error"),
                    },
                )
            ...
            patch_content = None
            if "patch" in patch_result and "patched_code" in patch_result["patch"]:
                patch_content = patch_result["patch"]["patched_code"]

            # Apply the patch if auto-apply is enabled
            applied = False
            if effective_config.auto_apply_patches and patch_id:
                try:
                    applied = await self.apply_patch(patch_id, trace_id)
                except Exception as e:
                    logger.warning(f"Failed to auto-apply patch: {e}")
            ...
            return SelfHealingResult(
                ...
                applied=applied,
                metadata={"original_result": patch_result},
            )
        except Exception as e:
            logger.exception(f"Failed to heal: {e}")
            return SelfHealingResult(success=False, metadata={"error": str(e)})
        finally:
            if span and self._trace_manager:
                self._trace_manager.end_span(span.span_id)
    ...
    def fix_error(self, error: Any, context: dict[str, Any]) -> dict[str, Any]:
        """
        Fix an error (synchronous version for compatibility).

        This method provides a synchronous interface for fixing errors,
        which is required by the ISelfHealingService interface in api.core.interfaces.
        ...
        """
        # For now, return a simple result indicating that async methods should be used
        return {
            "error_id": str(hash(str(error))),
            "fixed": False,
            "patch": None,
            "metadata": {
                "message": "Use async methods (heal, generate_patch) for actual self-healing operations",
