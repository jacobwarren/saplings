...
"""
Core agent module for Saplings.

This module provides the Agent class, which is the main entry point for using Saplings.
"""
...
from saplings.agent._internal.types import AgentBuilderProtocol, AgentProtocol
...
class Agent(AgentProtocol):
    ...
    def __init__(self, config: "AgentConfig") -> None:
        ...
        from saplings._internal.agent.container_config import configure_container
    ...
    @classmethod
    def builder(cls) -> AgentBuilderProtocol:
        """
        Get a builder for creating Agent instances.

        Returns
        -------
            AgentBuilderProtocol: Builder for creating Agent instances

        """
        # Use importlib to avoid circular imports
        builder_module = importlib.import_module("saplings._internal.agent.builder")
        return builder_module.AgentBuilder()
    ...
    def create(cls, provider: str, model_name: str, **kwargs) -> "Agent":
        ...
        builder = builder_module.AgentBuilder()
    ...
    def memory_store(self):
        ...
        return self._memory_manager.memory_store

    @memory_store.setter
    def memory_store(self, value):
        """Set the memory store."""
        # Store the memory store for later use
        # This avoids issues with the IMemoryManager interface not having a setter
        self._memory_store_value = value
        logger.info("Memory store value stored for later use")

    @property
    def dependency_graph(self):
        """Get the dependency graph."""
        return self._memory_manager.dependency_graph

    @dependency_graph.setter
    def dependency_graph(self, value):
        """Set the dependency graph."""
        # Store the dependency graph for later use
        # This avoids issues with the IMemoryManager interface not having a setter
        self._dependency_graph_value = value
        logger.info("Dependency graph value stored for later use")
