...
if TYPE_CHECKING:
    ...
    from scipy.sparse import spmatrix  # noqa: F401
...
class MaskVisualizer:
    ...
    def visualize_mask_sparsity(
        ...
        mask: np.ndarray | sp.spmatrix | list[dict[str, Any]],
        format: MaskFormat,
        mask_type: MaskType,  # pylint: disable=unused-argument
        output_path: str | None = None,
        title: str | None = None,
        show: bool = False,
    ...
    def visualize_mask_comparison(
        ...
        # Plot each mask
        for i, (mask, format, _mask_type, label) in enumerate(masks):  # pylint: disable=unused-variable
            # Convert mask to dense format
            dense_mask = self._convert_to_dense(mask, format)
    ...
    def _convert_to_dense(
        ...
        if format == MaskFormat.DENSE:
            ...
            # This else clause ensures the code below is reachable
