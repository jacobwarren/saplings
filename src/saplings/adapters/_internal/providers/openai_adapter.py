...
class OpenAIAdapter(LLM):
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        # Remove GASA-related parameters that OpenAI doesn't support
        _ = kwargs.pop("attention_mask", None)  # GASA attention mask not supported by OpenAI
        _ = kwargs.pop("position_mapping", None)  # GASA position mapping not supported by OpenAI
        ...
        # Import the run_in_executor function for proper timeout handling
        from saplings.core.resilience import run_in_executor

        # Create the completion with proper timeout handling
        # Extract timeout from kwargs if present
        timeout = kwargs.pop("timeout", None)

        # Use run_in_executor which properly handles timeouts
        response = await run_in_executor(
            self.client.chat.completions.create, timeout=timeout, **completion_args
        )
    ...
    async def generate_streaming(
        ...
        **kwargs: Any,
        ...
        # Create the completion with streaming and proper timeout handling
        # Extract timeout from kwargs if present
        timeout = kwargs.pop("timeout", None)
