...
class CostHeuristicConfig(BaseModel):
    ...
    base_cost_per_step: float = Field(
        0.01, description="Base cost for each plan step in USD"
    )
    ...
    retrieval_cost_per_doc: float = Field(
        0.001, description="Cost per document retrieved in USD"
    )
...
class PlannerConfig(BaseModel):
    ...
    max_steps: int = Field(
        10, description="Maximum number of steps in a plan"
    )
    min_steps: int = Field(
        1, description="Minimum number of steps in a plan"
    )
    total_budget: float = Field(
        1.0, description="Total budget for the plan in USD"
    )
    allow_budget_overflow: bool = Field(
        False, description="Whether to allow exceeding the budget"
    )
    ...
    enable_pruning: bool = Field(
        True, description="Whether to enable pruning of unnecessary steps"
    )
    enable_parallelization: bool = Field(
        True, description="Whether to enable parallel execution of independent steps"
    )
    enable_caching: bool = Field(
        True, description="Whether to enable caching of step results"
    )
    cache_dir: Optional[str] = Field(
        None, description="Directory to cache plan results"
    )
