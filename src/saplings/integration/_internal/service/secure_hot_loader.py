...
from saplings.tool_factory._internal.sandbox.sandbox import Sandbox, get_sandbox
...
class SecureHotLoader(HotLoader):
    ...
    def __init__(self, config: SecureHotLoaderConfig | None = None) -> None:
        ...
        if self.secure_config.enable_sandboxing:
            ...
            timeout = (
                self.secure_config.timeout_seconds
                if self.secure_config.timeout_seconds is not None
                else (self.secure_config.sandbox_timeout or 30.0)
            )
