...
- Trace visualization
- Event-based service registration
"""
...
from saplings.monitoring._internal.blame_graph import BlameEdge, BlameGraph, BlameNode
...
try:
    from saplings.monitoring._internal.test_trace_viewer import TestTraceViewer
except ImportError:
...
class MonitoringService(IMonitoringService):
    ...
    def __init__(
        ...
        self._service_hooks: dict[str, list[Callable]] = {}
    ...
    def register_service(self, service_name: str, service_instance: Any) -> None:
        ...
        # Execute any hooks registered for this service
        if service_name in self._service_hooks:
            for hook in self._service_hooks[service_name]:
                try:
                    hook(service_instance)
                except Exception as e:
                    logger.error(f"Error executing hook for service {service_name}: {e}")

        # Create a blame node for the service
        self.create_blame_node(service_name, "service", {"registered": True})
    ...
    def register_service_hook(self, service_name: str, hook: Callable) -> None:
        ...
        # If the service is already registered, execute the hook immediately
        if service_name in self._registered_services:
            try:
                hook(self._registered_services[service_name])
            except Exception as e:
                logger.error(f"Error executing hook for service {service_name}: {e}")
