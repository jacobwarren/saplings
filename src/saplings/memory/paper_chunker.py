        ...
        if len(section_content) > max_chunk_size:
            ...
            for i, chunk_content in enumerate(paragraph_chunks):
                ...
                if isinstance(document.metadata, DocumentMetadata):
                    ...
                    custom = dict(get_custom_metadata(document.metadata))
...
def build_section_relationships(chunks: list[Document]) -> list[dict]:
    ...
    # Connect page sections sequentially
    page_sections = [section for section in section_map if section.startswith("Page ")]
    if page_sections:
        page_sections.sort(
        ...
        for i in range(len(page_sections) - 1):
            for source_id in section_map[page_sections[i]]:
                for target_id in section_map[page_sections[i + 1]]:
                    relationships.append(
                        {
                            "source_id": source_id,
                            "target_id": target_id,
