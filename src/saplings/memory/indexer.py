...
class IndexerRegistry:
    ...
    def __new__(cls):
        ...
        Returns:
            IndexerRegistry: Singleton instance
        """
        if cls._instance is None:
            cls._instance = super(IndexerRegistry, cls).__new__(cls)
    ...
    def register_indexer(self, name: str, indexer_class: type) -> None:
        ...
        if not issubclass(indexer_class, Indexer):
            raise TypeError(f"Indexer class must be a subclass of Indexer, got {indexer_class}")
...
def get_indexer(
    ...
    try:
        return IndexerRegistry().get_indexer(name, config)
    except ValueError:
