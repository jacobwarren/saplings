...
class GASABenchmark:
    ...
    def __init__(
        ...
        self.blame_graph = BlameGraph(
            trace_manager=self.trace_manager, config=self.monitoring_config
        )
    ...
    async def run_benchmark(self):
        ...
        # Create model - parse the URI format
        if "://" in self.model_uri:
            # Handle URIs like "local://llama3?model_path=/path/to/model"
            provider, rest = self.model_uri.split("://", 1)
            if "?" in rest:
                model_name, params_str = rest.split("?", 1)
                # Parse params if needed
                params = dict(item.split("=") for item in params_str.split("&"))
                model = LLM.create(provider=provider, model=model_name, **params)
            else:
                model_name = rest
                model = LLM.create(provider=provider, model=model_name)
        else:
            # Simple format, assume transformers provider
            model = LLM.create(provider="transformers", model=self.model_uri)
    ...
    async def _run_configuration(
        ...
        if enable_gasa and max_hops is not None:
            ...
            # Calculate estimated FLOP reduction (assumes attention is O(n²))
            # The reduction scales with the number of zeros in the mask
            flop_reduction = mask_sparsity
        ...
        result = {
            ...
            "flop_reduction": float(flop_reduction) if flop_reduction is not None else None,
            "raw_latencies_ms": [float(l) for l in latencies],
        }

        flop_reduction_str = (
            f", flop_reduction={flop_reduction:.2%}" if flop_reduction is not None else ""
        )
        print(
            f"  Results: avg_latency={avg_latency:.2f}ms, mask_sparsity={mask_sparsity:.2%}{flop_reduction_str}"
        )
    ...
    def _create_test_documents(self, num_documents: int) -> Tuple[List[Document], DependencyGraph]:
        ...
        for i in range(len(nodes)):
            ...
            # Threshold for "larger document sets"
            LARGE_DOC_THRESHOLD = 5
            # Interval for connecting documents
            CONNECT_INTERVAL = 3

            if num_documents > LARGE_DOC_THRESHOLD:
                # Connect every CONNECT_INTERVAL document
                if i % CONNECT_INTERVAL == 0 and i + CONNECT_INTERVAL < len(nodes):
                    graph.add_relationship(
                        nodes[i].id, nodes[i + CONNECT_INTERVAL].id, "references"
                    )
    ...
    def _generate_report(self, results: Dict) -> None:
        ...
        with open(filename, "w") as f:
                ...
                if baseline:
                    ...
                    for config in configs:
                        if config["enable_gasa"]:
                            ...
                            sparsity = (
                                config["mask_sparsity"] * 100
                                if config["mask_sparsity"] is not None
                                else "N/A"
                            )
                            flop_reduction = (
                                config["flop_reduction"] * 100
                                if config["flop_reduction"] is not None
                                else "N/A"
                            )

                            f.write(f"- **GASA (h={config['max_hops']})**: {latency:.2f} ms ")
                            f.write(
                                f"({improvement:.2f}% improvement, {sparsity:.2f}% sparsity, {flop_reduction:.2f}% FLOP reduction)\n"
                            )
            ...
            # Create a table
            f.write(
                "| Configuration | Doc Size | Latency (ms) | Memory (MB) | Sparsity | FLOP Reduction |\n"
            )
            f.write(
                "|--------------|----------|--------------|-------------|----------|---------------|\n"
            )
            ...
            for config in results["configurations"]:
                ...
                sparsity = (
                    f"{config['mask_sparsity']*100:.2f}%"
                    if config["mask_sparsity"] is not None
                    else "N/A"
                )
                flop_reduction = (
                    f"{config['flop_reduction']*100:.2f}%"
                    if config["flop_reduction"] is not None
                    else "N/A"
                )

                f.write(
                    f"| {name} | {doc_size} | {latency:.2f} | {memory:.2f} | {sparsity} | {flop_reduction} |\n"
                )
            ...
            f.write(
                f"with open('{os.path.basename(filename).replace('_report', '')}', 'r') as f:\n"
            )
            ...
            f.write(
                "        config = next((c for c in results['configurations'] if c['name'].startswith(config_type) and c['doc_size'] == doc_size), None)\n"
            )
            f.write("        if config:\n")
...
async def main():
    ...
    parser.add_argument(
        "--model", type=str, default="local://llama3?model_path=/path/to/model", help="Model URI"
    )
    parser.add_argument(
        "--output-dir", type=str, default="./benchmark_results", help="Output directory"
    )
    parser.add_argument(
        "--num-runs", type=int, default=5, help="Number of runs for each configuration"
    )
    parser.add_argument(
        "--document-sizes",
        type=int,
    ...
    parser.add_argument(
        "--max-hops", type=int, nargs="+", default=[1, 2, 3], help="List of max_hops values to test"
    )
